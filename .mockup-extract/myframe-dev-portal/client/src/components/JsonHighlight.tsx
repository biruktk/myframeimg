  ...
  while (i < s.length) {
    ...
    if (s[i] === '"') {
      ...
      while (i < s.length && s[i] !== '"') {
        if (s[i] === "\\") { w += s[i]; i++; if (i < s.length) { w += s[i]; i++; } }
        else { w += s[i]; i++; }
      }
      if (i < s.length) w += s[i++];
      let j = i;
    ...
    if (/\d/.test(s[i]) || (s[i] === "-" && i + 1 < s.length && /\d/.test(s[i + 1]))) {
      let w = "";
      if (s[i] === "-") { w += s[i]; i++; }
      while (i < s.length && /\d/.test(s[i])) { w += s[i]; i++; }
      if (i < s.length && s[i] === ".") { w += s[i]; i++; while (i < s.length && /\d/.test(s[i])) { w += s[i]; i++; } }
      tokens.push({ type: "number", value: w });
      continue;
    }
    const rest = s.slice(i);
    if (rest.startsWith("true")) { tokens.push({ type: "boolean", value: "true" }); i += 4; continue; }
    if (rest.startsWith("false")) { tokens.push({ type: "boolean", value: "false" }); i += 5; continue; }
    if (rest.startsWith("null")) { tokens.push({ type: "null", value: "null" }); i += 4; continue; }
    tokens.push({ type: "punctuation", value: s[i++] });
  }
  return tokens;
}
...
const styleMap: Record<string, string> = {
  key: "text-[#c678dd] dark:text-[#c678dd]",
  string: "text-[#98c379] dark:text-[#98c379]",
  number: "text-[#d19a66] dark:text-[#d19a66]",
  boolean: "text-[#e5c07b] dark:text-[#e5c07b]",
  null: "text-[#5c6370] dark:text-[#5c6370]",
    ...
    <pre className={`font-mono text-xs overflow-x-auto whitespace-pre break-words ${className}`}>
      <code>
        {tokens.map((t, i) => {
          ...
          return <span key={i} className={styleMap[t.type] || styleMap.punctuation}>{t.value}</span>;
        })}
      </code>
    </pre>
  );
