...
import { DevLiveSample } from "./dev-live-snapshot";
import { useDevStore } from "./store/use-dev-store";

export function DevDescription() {
  const { selectedSection, selectedApi, selectedItemId, allApis } = useDevStore();
    ...
    return (
      <div className="p-6 md:p-8">
        ...
        {relatedApis.length > 0 && (
          <div>
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Related APIs</h3>
            <div className="space-y-2">
              {relatedApis.map((api) => (
                <div key={api.id} className="flex items-center gap-3 p-3 rounded-lg border border-border bg-secondary/40">
                  <span
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground truncate">{api.proxyPath}</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => copyText(api.proxyPath)}
    ...
    return (
      <div className="p-6 md:p-8">
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          ...
          <code className="text-sm text-muted-foreground break-all">{selectedApi.proxyPath}</code>
          <span
            className={`text-[10px] px-2 py-0.5 rounded-full ${selectedApi.auth === "admin" ? "bg-amber-500/15 text-amber-500" : "bg-secondary text-muted-foreground"}`}
          >
            {selectedApi.auth === "admin" ? "admin" : "public"}
          </span>
        </div>
        ...
        {selectedApi.requestNote && (
          <div className="p-3 rounded-lg bg-secondary/70 border border-border text-sm text-muted-foreground mb-4">
            <strong className="text-foreground">Note:</strong> {selectedApi.requestNote}
          </div>
        )}
        {selectedApi.responseNote && (
          <div className="p-3 rounded-lg bg-secondary/40 border border-border text-sm text-muted-foreground mb-4 font-mono text-[12px]">
            <strong className="text-foreground font-sans text-sm">Response shape:</strong> {selectedApi.responseNote}
          </div>
        )}
        <DevLiveSample apiId={selectedApi.id} />
        <div className="mt-6">
          <h3 className="text-sm font-semibold mb-2 text-muted-foreground uppercase tracking-wider">Parameters</h3>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border">
                  {selectedApi.params.map((param) => (
                    <tr key={param.name} className="hover:bg-secondary/50">
