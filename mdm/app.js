...
// ────────────────────────────────────────────────────────────────
// Firmware OTA — shared with mobile app + backend (same MQTT + /firmware URL)
// ────────────────────────────────────────────────────────────────

function mdmLatestFirmwareTag() {
  var lf = window.__mdmBootstrapPayload && window.__mdmBootstrapPayload.latestFirmware;
  var v = lf && lf.version ? String(lf.version) : '1.4.3';
  return v.indexOf('v') === 0 ? v : 'v' + v;
}

function mdmLatestFirmwarePlain() {
  return mdmLatestFirmwareTag().replace(/^v/i, '');
}

async function mdmPushOtaToDevice(deviceId, version) {
  var ver = String(version || mdmLatestFirmwarePlain()).replace(/^v/i, '');
  var res = await fetch('/api/admin/frames/' + encodeURIComponent(deviceId) + '/ota', {
    method: 'POST',
...
async function mdmPushOtaRollout(devices, version) {
  var tag = mdmLatestFirmwareTag();
  var targets = (devices || []).filter(function (d) {
    return d && d.id && d.firmware !== tag && d.status !== 'offline';
  });
  if (!targets.length) return { ok: 0, fail: 0, skipped: (devices || []).length };
  var ok = 0;
  var fail = 0;
  for (var i = 0; i < targets.length; i++) {
    var r = await mdmPushOtaToDevice(targets[i].id, version);
    if (r.ok) ok++; else fail++;
  }
  return { ok: ok, fail: fail, skipped: (devices || []).length - targets.length };
}
  ...
  main.addEventListener('click', (e) => {
    ...
    switch (action) {
      ...
      case 'bulk-update':
        if (store) {
          void (async function () {
            var latest = mdmLatestFirmwareTag();
            var outdated = store.getDevices().filter(function (d) { return d.firmware !== latest; });
            if (!outdated.length) {
              toast.info('All devices already on ' + latest);
              modal.hide();
              return;
            }
            toast.info('⬆️ Pushing ' + latest + ' to ' + outdated.length + ' device(s)…');
            var result = await mdmPushOtaRollout(outdated, mdmLatestFirmwarePlain());
            if (window.__mdmRetryBootstrap) await window.__mdmRetryBootstrap();
            modal.hide();
            renderDevices();
            renderOta();
            if (result.fail) {
              toast.warning('OTA queued for ' + result.ok + ', failed ' + result.fail + (result.skipped ? ', skipped ' + result.skipped + ' offline' : ''));
            } else {
              toast.success('⬆️ OTA queued for ' + result.ok + ' device(s) → ' + latest);
            }
          })();
        }
        break;
      ...
      case 'start-ota-rollout':
        if (store) {
          void (async function () {
            ...
            var outdated = store.getDevices().filter(function (d) { return d.firmware !== latest; });
            if (!outdated.length) {
              toast.info('Fleet is already on ' + latest);
              navigate('ota');
              return;
            }
            toast.info('⬆️ Starting OTA roll-out to ' + latest + '…');
            var result = await mdmPushOtaRollout(outdated, mdmLatestFirmwarePlain());
            if (window.__mdmRetryBootstrap) await window.__mdmRetryBootstrap();
            navigate('ota');
            renderOta();
            if (result.fail) {
              toast.warning('OTA roll-out: ' + result.ok + ' queued, ' + result.fail + ' failed');
            } else {
              toast.success('⬆️ OTA roll-out queued for ' + result.ok + ' device(s)');
            }
          })();
        }
        break;
      case 'ota-update-device':
        if (store && target) {
          var deviceId = target.getAttribute('data-id');
          if (!deviceId) break;
          void (async function () {
            toast.info('⬆️ Queuing firmware update…');
            var r = await mdmPushOtaToDevice(deviceId);
            if (window.__mdmRetryBootstrap) await window.__mdmRetryBootstrap();
            renderOta();
            if (r.ok) toast.success('⬆️ OTA queued for device');
            else toast.error((r.data && r.data.message) || (r.data && r.data.error) || 'OTA push failed');
          })();
        }
        break;
      case 'invite-user':
...
function renderAnalytics() {
  ...
  if (statEls.length >= 4) {
    ...
    var onFirmware = devices.filter(function(d) { return d.firmware === mdmLatestFirmwareTag(); }).length;
...
function renderOta() {
  ...
  const devices = store.getDevices();
  const latestTag = mdmLatestFirmwareTag();
  const lf = window.__mdmBootstrapPayload && window.__mdmBootstrapPayload.latestFirmware;
  const latestStat = document.querySelector('#page-ota .stat-val[style*="--green"]');
  if (latestStat) latestStat.textContent = latestTag;
  const releaseTitle = document.querySelector('#page-ota .ch-title');
  if (releaseTitle && lf && lf.releaseNotes) {
    releaseTitle.textContent = 'Release Notes — ' + latestTag;
  }
  const tbody = document.querySelector('#page-ota table tbody');
  if (!tbody) return;

  tbody.innerHTML = devices.slice(0, 12).map(d => {
    const isUpToDate = d.firmware === latestTag;
    const statusPill = isUpToDate ? 'pill-g' : d.status === 'offline' ? 'pill-r' : 'pill-a';
    const statusText = isUpToDate ? 'Up to date' : d.status === 'offline' ? 'Outdated · Offline' : (d.otaStatus === 'updating' ? 'Updating…' : 'Update available');
    return `<tr>
      <td>${escapeHtml(d.name)}</td>
      <td><span class="tag">${d.firmware}</span></td>
      <td><span class="tag">${latestTag}</span></td>
      <td><span class="pill ${statusPill}">${statusText}</span></td>
      <td>${isUpToDate ? '—' : `<button class="btn btn-p" style="font-size:11px;padding:3px 8px;" type="button" data-action="ota-update-device" data-id="${escapeHtml(d.id)}">Update</button>`}</td>
    </tr>`;
  }).join('');
}
