...
export async function DELETE(req: NextRequest, ctx: Ctx) {
  ...
  try {
    const res = await fetch(`${getMyframeApiBase()}/api/admin/uploads/${encodeURIComponent(id)}`, {
      method: "DELETE",
    ...
    return NextResponse.json(
      { ok: false, error: e instanceof Error ? e.message : "admin_upload_delete_proxy_failed" },
      { status: 502 },
    );
  }
}
