...
export async function PUT(req: NextRequest, { params }: Props) {
  ...
  try {
    ...
    const res = await fetch(`${getMyframeApiBase()}/api/admin/orders/${encodeURIComponent(id)}`, {
      method: "PUT",
    ...
    return NextResponse.json(
      { ok: false, error: e instanceof Error ? e.message : "admin_order_update_proxy_failed" },
      { status: 502 },
    );
  }
}
