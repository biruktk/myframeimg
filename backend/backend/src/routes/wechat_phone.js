...
exports.wechatPhoneRouter = void 0;
const crypto_1 = __importDefault(require("crypto"));
const express_1 = __importDefault(require("express"));
const store_1 = require("../db/store");
const app_user_jwt_1 = require("../services/app_user_jwt");
exports.wechatPhoneRouter = express_1.default.Router();
exports.wechatPhoneRouter.use(express_1.default.json({ limit: "256kb" }));
function env(name) {
...
async function getOpenId(appid, secret, loginCode) {
    if (!loginCode)
        return {};
...
async function getPhoneNumber(accessToken, phoneCode) {
...
function upsertWechatPhoneUser(phone, openid) {
    ...
    if (existing) {
        store_1.db.mutate((draft) => {
            draft.users = draft.users.map((u) => (u.id === existing.id ? { ...u, lastSeenAtMs: now } : u));
    ...
    const id = "usr_wx_" + phoneHash.slice(0, 12) + "_" + crypto_1.default.randomBytes(3).toString("hex");
...
exports.wechatPhoneRouter.post("/wechat/phone-login", async (req, res) => {
    try {
        ...
        const user = upsertWechatPhoneUser(phone, session.openid);
        const token = (0, app_user_jwt_1.signUserJwt)(user.id, user.email);
        res.json({
            ok: true,
            token,
