...
const geo_lookup_1 = require("../../../lib/geo-lookup");
const shipping_estimate_1 = require("../../../lib/shipping-estimate");
...
exports.publicSiteRouter.get("/public/location", async (req, res) => {
    const acceptLanguage = String(req.header("accept-language") ?? "");
    const ip = getClientIp(req);
    const geo = await (0, geo_lookup_1.lookupGeoByIp)(ip, acceptLanguage);
    const shipping = (0, shipping_estimate_1.shippingEstimateForCountry)(geo.countryCode || "US");
    res.json({
        ...geo,
        shipping,
    });
...
exports.publicSiteRouter.get("/public/shipping-estimate", (req, res) => {
    ...
    const estimate = (0, shipping_estimate_1.shippingEstimateForCountry)(country || fallback || "US");
...
exports.publicSiteRouter.post("/public/contact-messages", (req, res) => {
    const name = String(req.body?.name ?? "").trim();
    const email = String(req.body?.email ?? "").trim().toLowerCase();
    ...
    const id = `msg_${Date.now()}_${crypto_1.default.randomBytes(3).toString("hex")}`;
    store_1.db.mutate((draft) => {
        if (!draft.marketingCms)
...
exports.publicSiteRouter.post("/public/orders", (req, res) => {
    ...
    const shipping = Math.max(0, Number(req.body?.shipping ?? (0, shipping_estimate_1.shippingEstimateForCountry)(String(country || "")).price) || 0);
