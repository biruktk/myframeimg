...
async function sendPasswordResetEmail(email, rawToken) {
    ...
    if (user) {
        await (0, firebase_admin_1.sendPushToUser)(user.id, "Password Reset Requested", "A password reset was requested for your account.");
    }
}
async function sendPasswordChangedNotification(email) {
    ...
    const user = data.users.find((u) => u.email.toLowerCase() === email.toLowerCase());
    if (user) {
        await (0, firebase_admin_1.sendPushToUser)(user.id, "Password Changed", "Your MyFrame account password was changed successfully.");
    }
}
