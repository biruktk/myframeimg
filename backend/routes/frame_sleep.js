...
exports.frameSleepRouter = frameSleepRouter;
...
var TIME_RE = /^\d{2}:\d{2}$/;
function frameSleepRouter() {
    var router = (0, express_2.Router)();
    router.use(express_1.default.json({ limit: "128kb" }));
    router.get("/frames/:mac/sleep-config", function (req, res) {
        var u = (0, app_user_jwt_1.verifyUserJwtBearer)(req);
        if (!u) {
    ...
    router.post("/frames/:mac/sleep-config", function (req, res) {
        ...
        store_1.db.mutate(function (draft) {
            ...
            if (frame) {
                ...
                if (frame.stationMac)
                    publishMac = frame.stationMac;
            }
        });
        (0, frame_mqtt_1.publishSleepConfig)(publishMac, sleepConfig).catch(function () { });
        if ((0, frame_mqtt_1.isMqttConnected)()) {
            var immAction = enabled ? "sleep" : "wake";
            (0, frame_mqtt_1.publishMqttAction)(publishMac, immAction).catch(function () { });
        }
