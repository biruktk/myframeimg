...
function renderStatusCard(title, message, isSuccess) {
...
function sendJsonOrHtml(req, res, status, jsonBody, htmlTitle, htmlBody) {
...
exports.authRouter.use(express_1.default.urlencoded({ extended: true }));
exports.authRouter.post("/auth/register", (req, res) => {
    ...
    const smtpAvailable = (0, email_service_1.isSmtpConfigured)();
    store_1.db.mutate((draft) => {
        ...
        if (smtpAvailable) {
            ...
            draft.emailVerifications.push({
                id: `emailver_${Date.now()}_${crypto_1.default.randomBytes(4).toString("hex")}`,
                userId: id,
                email,
                tokenHash,
            ...
            void (0, email_service_1.sendVerificationEmail)(email, rawToken);
        }
...
exports.authRouter.post("/auth/google", (req, res) => {
    void (0, google_auth_post_1.handleGoogleAuthPost)(req, res);
});
exports.authRouter.post("/auth/apple", (req, res) => {
    void (0, apple_auth_post_1.handleAppleAuthPost)(req, res);
});
...
exports.authRouter.post("/auth/fcm-token", (req, res) => {
    const authed = (0, app_user_jwt_1.verifyUserJwtBearer)(req);
    if (!authed) {
...
exports.authRouter.get("/auth/verify-email", (req, res) => {
    try {
        ...
        store_1.db.mutate((draft) => {
            draft.emailVerifications = draft.emailVerifications.map((v) => v.id === record.id ? { ...v, usedAtMs: Date.now() } : v);
            draft.users = draft.users.map((u) => {
...
const forgotPasswordBucket = new Map();
exports.authRouter.post("/auth/forgot-password", (req, res) => {
    const email = normalizeEmail(req.body?.email);
    ...
    if (!bucket || now >= bucket.resetAtMs) {
        forgotPasswordBucket.set(key, { count: 1, resetAtMs: now + 60000 });
    }
    else {
        bucket.count += 1;
    }
    ...
    store_1.db.mutate((draft) => {
        draft.passwordResets.push({
            id: `pwreset_${Date.now()}_${crypto_1.default.randomBytes(4).toString("hex")}`,
            userId: user.id,
            emailHash: crypto_1.default.createHash("sha256").update(email).digest("hex"),
            tokenHash,
            expiresAtMs,
            usedAtMs: null,
            createdAtMs: Date.now(),
    ...
    void (0, email_service_1.sendPasswordResetEmail)(email, rawToken);
    res.json({
        ok: true,
        message: "If that email is registered, a reset link has been sent.",
    });
});
exports.authRouter.post("/auth/resend-verification", (req, res) => {
    const email = normalizeEmail(req.body?.email);
    if (!email || !email.includes("@")) {
        res.status(400).json({ ok: false, error: "invalid_email" });
        return;
...
exports.authRouter.get("/auth/reset-password/validate", (req, res) => {
    const rawToken = String(req.query?.token ?? "").trim();
    if (!rawToken) {
...
exports.authRouter.post("/auth/reset-password", (req, res) => {
    try {
        ...
        store_1.db.mutate((draft) => {
            draft.passwordResets = draft.passwordResets.map((r) => r.id === record.id ? { ...r, usedAtMs: Date.now() } : r);
            draft.users = draft.users.map((u) => {
                if (u.id !== record.userId)
                    return u;
        ...
        const user = store_1.db.read().users.find((u) => u.id === record.userId);
        if (user) {
            (0, email_service_1.sendPasswordChangedNotification)(user.email).catch((e) => {
...
exports.authRouter.get("/auth/reset-password", (req, res) => {
    try {
        const rawToken = String(req.query?.token ?? "").trim();
        if (!rawToken) {
            const accept = (req.headers.accept || "").toLowerCase();
...
exports.authRouter.use((err, req, res, _next) => {
