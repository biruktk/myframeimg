...
function upsertWechatPhoneUser(phone, openid, unionid) {
    ...
    let existing = data.users.find((u) => unionid && u.wechatUnionId === unionid) ??
    ...
    if (existing) {
        store_1.db.mutate((draft) => {
            draft.users = draft.users.map((u) => u.id === existing.id
                ? {
                    ...u,
                    lastSeenAtMs: now,
